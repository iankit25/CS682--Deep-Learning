...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # Compute the loss
    loss = None
    scores = (np.exp(scores.T-(np.max(scores,axis=1).T)).T)
    ...
    temp1[np.arange(0,F1.shape[0]),y]=1
    dW2 = F1.T.dot(scores - temp1)
    dW2/=X.shape[0]
    dW2 += reg*W2
    db2= np.sum((scores-temp1)/X.shape[0], axis=0)
    dX = ((scores - temp1)/X.shape[0]).dot(W2.T)
    ...
    temp = F1
    temp[temp>0]=1
    dX = np.multiply(temp,dX)
    db1 = np.sum(dX,axis=0)
    
    grads['W1']= X.T.dot(dX)+reg*W1
    grads['b1']= db1
